...
REM if NOT EXIST "%appdata%\NZ-DOS\system" goto install
goto Bootmenu
:install
...
echo Programs :
echo -----------
echo NADB     NaDB      2K       /usr/share/NaDB
goto EnterCmd
echo.
:ls
...
if NOT EXIST "%appdata%\NZ-DOS\home\USER\.nadb.d\databases" mkdir "%appdata%\NZ-DOS\home\USER\.nadb.d\databases"
goto NadbAfterStart
:NadbAfterStart
set prgpath=%cd%
...
echo CREATOR_HOSTNAME=%computername%>> %dbmgruseraddname%/.properties
...
echo Actual user : "USER@NzDos"
echo Actual path : "%dir%"
echo This version of NaDB is activated by NzDos\root
goto NadbPhome
:NadbDlist
echo List of all databases on NzDos for user USER :
dir %dbpath% /b
goto NadbPhome
:NadbKstop
